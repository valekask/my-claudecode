...
1. **Proposal** (manual) - create a proposal file with task description. Drop reference assets (mockups, screenshots, diagrams) if any.
2. **Brainstorm** - turn the proposal into an approved spec
3. **Writing-plan** - decompose the spec into a bite-sized implementation plan
4. **Execute** - implement the plan with a fresh subagent per task in isolated context
5. **Review** - checklist-driven quality gate
6. **Formatting** - apply prettier and code organization
7. **Create PR** (manual)
...
Turns a proposal into an approved spec — a contract specifying **what + why** (goals, constraints, architecture). The _how_ is discovered during plan writing and execution.
